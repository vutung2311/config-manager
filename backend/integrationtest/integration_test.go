...
func waitForPocketBaseReady() error {
	...
	for i := 0; i < maxRetries; i++ {
		...
		resp, err := http.Get("http://localhost:8080/_/")
...
func testPocketBaseAuthenticationAndCollections() error {
	...
	resp, err := http.Post("http://localhost:8080/api/collections/_superusers/auth-with-password",
	...
	req, err := http.NewRequest("GET", "http://localhost:8080/api/collections", nil)
	if err != nil {
